...
public class HandlerMethodMetaArgsFactory {
	private final static String module = HandlerMethodMetaArgsFactory.class
			.getName();
	...
	public MethodMetaArgs createGetMethod(HandlerMetaDef handlerMetaDef,
		...
		if (keyValue == null) {
			Debug.logError("[JdonFramework] not found model's key value:"+ handlerMetaDef.getModelMapping().getKeyName()+"=? in request parameters", module);
		}
	...
	public MethodMetaArgs createCreateMethod(HandlerMetaDef handlerMetaDef,
		...
		if (p_methodName == null) {
			Debug
					.logError(
							"[JdonFramework] not configure the createMethod parameter: <createMethod name=XXXXX /> ",
							module);
		}
		return createCRUDMethodMetaArgs(p_methodName, em);
	}
	...
	public MethodMetaArgs createUpdateMethod(HandlerMetaDef handlerMetaDef,
		...
		if (p_methodName == null) {
			Debug
					.logError(
							"[JdonFramework] not configure the updateMethod parameter: <updateMethod name=XXXXX /> ",
							module);
		}
		return createCRUDMethodMetaArgs(p_methodName, em);
	}
	...
	public MethodMetaArgs createDirectMethod(String methodName,
			Object[] methodParams) {
		MethodMetaArgs methodMetaArgs = null;
		try {
			if (methodName == null)
				throw new Exception(
						"no configure method value, but now you call it: ");
			...
			for (int i = 0; i < methodParams.length; i++) {
				...
				Debug.logVerbose("[JdonFramework], parameter type:"
						+ paramTypes[i] + " and parameter value:" + p_args[i],
						module);
			}
			methodMetaArgs = new MethodMetaArgs(methodName, paramTypes, p_args);

		} catch (Exception ex) {
			Debug.logError("[JdonFramework] createDirectMethod error: " + ex,
					module);
		}
		return methodMetaArgs;
	}

	private MethodMetaArgs createCRUDMethodMetaArgs(String p_methodName,
			Object args) {
		MethodMetaArgs methodMetaArgs = null;
		try {
			...
			methodMetaArgs = new MethodMetaArgs(p_methodName, paramTypes,
					p_args);
			Debug.logVerbose("[JdonFramework] construct "
					+ methodMetaArgs.getMethodName() + ", parameter type:"
					+ paramTypes[0], module);
		} catch (Exception ex) {
			Debug.logError("[JdonFramework] create CRUD method error: " + ex,
					module);
		}
		return methodMetaArgs;
	}
