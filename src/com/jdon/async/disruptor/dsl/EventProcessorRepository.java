...
class EventProcessorRepository<T> implements Iterable<EventProcessorInfo<T>> {
	...
	public void clear() {
		Map mydisruptors = new HashMap(this.eventProcessorInfoByEventProcessor);
		this.eventProcessorInfoByEventProcessor.clear();
		Iterator keys = mydisruptors.keySet().iterator();
		while (keys.hasNext()) {
			Object key = keys.next();
			EventProcessorInfo epinfo = (EventProcessorInfo) mydisruptors.get(key);
			try {
				epinfo.stop();
			} catch (Exception e) {
			}
		...
		mydisruptors = new HashMap(this.eventProcessorInfoByHandler);
		this.eventProcessorInfoByHandler.clear();
		keys = mydisruptors.keySet().iterator();
		while (keys.hasNext()) {
			Object key = keys.next();
			EventProcessorInfo epinfo = (EventProcessorInfo) mydisruptors.get(key);
			try {
