...
public class EventHandlerGroup<T> {
	private final JdonDisruptor<T> disruptor;
	private final EventProcessorRepository<T> eventProcessorRepository;
	private final EventProcessor[] eventProcessors;

	EventHandlerGroup(final JdonDisruptor<T> disruptor, final EventProcessorRepository<T> eventProcessorRepository,
	 ...
	 * @return a new EventHandlerGroup combining the existing and new handlers
	...
	/**
	 * Set up batch handlers to handleEventException events from the ring
	 * buffer. These handlers will only process events after every
	 * {@link EventProcessor} in this group has processed the event.
	 * 
	 * <p>
	 * This method is generally used as part of a chain. For example if the
	 * handler <code>A</code> must process events before handler <code>B</code>:
