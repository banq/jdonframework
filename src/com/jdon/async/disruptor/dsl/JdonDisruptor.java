...
public class JdonDisruptor<T> {
	...
	private EventPublisher<T> eventPublisher;
	 ...
	 */
	public JdonDisruptor(final EventFactory<T> eventFactory, final int ringBufferSize, final ExecutorService executor) {
	 ...
	 */
	public JdonDisruptor(final EventFactory<T> eventFactory, final ExecutorService executor, final ClaimStrategy claimStrategy,
			final WaitStrategy waitStrategy) {
		this(new RingBuffer<T>(eventFactory, claimStrategy, waitStrategy), executor);
	}

	private JdonDisruptor(final RingBuffer<T> ringBuffer, final ExecutorService executor) {
	...
	/**
	 * Set up event handlers to handleEventException events from the ring
	 * buffer. These handlers will process events as soon as they become
	...
	/**
	 * Set up custom event processors to handleEventException events from the
	 * ring buffer. The Disruptor will automatically start this processors when
	 * {@link #start()} is called.
	 * 
	 * @param processors
	 *            the event processors that will process events.
	 * @return a {@link EventHandlerGroup} that can be used to chain
	 *         dependencies.
	 */
	 ...
	 *            the exception handler to use for any future
	 *            {@link EventProcessor}.
	 */
	 ...
	 * 
	 * @param eventHandler
	 *            the event handler to set a different exception handler for.
	 ...
	 * 
	 * @param handlers
	 *            the event handlers, previously set up with
	 *            {@link #handleEventsWith(com.lmax.disruptor.EventHandler[])},
	 *            that will form the barrier for subsequent handlers or
	 *            processors.
	 ...
	 * 
	 * @param processors
	 *            the event processors, previously set up with
	 *            {@link #handleEventsWith(com.lmax.disruptor.EventProcessor...)}
	 *            , that will form the barrier for subsequent handlers or
	 *            processors.
	 * @return an {@link EventHandlerGroup} that can be used to setup a
	 *         {@link SequenceBarrier} over hte specified event processors.
	 ...
	 * be processed by the slowest event processor. This method must only be
	 * called once after all event processors have been added.
	 * 
	...
	public void shutdown() {
		...
		halt();
		started.set(false);
		this.eventProcessorRepository.clear();
		this.eventProcessorRepository = null;
	...
	/**
	 * The the {@link RingBuffer} used by this Disruptor. This is useful for
	 ...
	 * 
	 * @param handler
	 *            the handler to get the barrier for.
	...
	EventHandlerGroup<T> createEventProcessors(final EventProcessor[] barrierEventProcessors, final EventHandler<T>[] eventHandlers) {
		...
		for (int i = 0, eventHandlersLength = eventHandlers.length; i < eventHandlersLength; i++) {
			...
			// replace
			final JdonBatchEventProcessor<T> batchEventProcessor = new JdonBatchEventProcessor<T>(ringBuffer, barrier, eventHandler);
