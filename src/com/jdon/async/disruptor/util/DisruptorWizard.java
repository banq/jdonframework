 ...
 * 
 * <pre>
 * <code> DisruptorWizard<MyEvent> dw = new DisruptorWizard<MyEvent>(MyEvent.FACTORY, 32, Executors.newCachedThreadPool());
 * EventHandler<MyEvent> handler1 = new EventHandler<MyEvent>() { ... };
 * EventHandler<MyEvent> handler2 = new EventHandler<MyEvent>() { ... };
 * dw.handleEventsWith(handler1);
 * dw.after(handler1).handleEventsWith(handler2);
 * <p/>
 * RingBuffer ringBuffer = dw.start();</code>
 * </pre>
 * 
 * @param <T>
 *            the type of {@link AbstractEvent} used.
 */
public class DisruptorWizard<T extends AbstractEvent> {
	 ...
	 */
	public DisruptorWizard(final EventFactory<T> eventFactory, final int ringBufferSize, final Executor executor,
			final ClaimStrategy.Option claimStrategy, final WaitStrategy.Option waitStrategy) {
	 ...
	 *            the exception handler to use for any future eventprocessors.
	 */
	 ...
	 * @return an {@link EventHandlerGroup} that can be used to setup a handler
	 *         barrier over the specified event handlers.
	 */
	...
	/**
	 * Get the dependency barrier used by a specific handler. Note that the
	 * dependency barrier may be shared by multiple event handlers.
	 * 
	 * @param handler
	 ...
	 * event processors created via this wizard.
	 */
	public void halt() {
